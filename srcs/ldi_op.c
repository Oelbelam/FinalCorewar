...
/*   Updated: 2020/10/18 17:36:16 by oelbelam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ldi_arg2_execut(t_vm *vm, t_proc **prcs, int *index, short value)
{
	...
	if (check_reg(vm, (*prcs)->cur_pos + index[0]))
	{
		(*prcs)->r[vm->arena[i_mod((*prcs)->cur_pos +
		index[0])] - 1] = convert_pos(vm,
		i_mod((*prcs)->cur_pos - 2 + idx_mod(index[1])), 4);
	}
	return (index[0]);
}

int			ldi_execute(t_vm *vm, t_proc **pr)
{
	int		crt_p;
	short	tmp_idx;
	int32_t	tmp_idx2;
	...
	{
		if (check_reg(vm, (*pr)->cur_pos) == 1)
			tmp_idx2 = (*pr)->r[vm->arena[(*pr)->cur_pos] - 1];
		else
			return ((*pr)->args.s_arg1 +
			(*pr)->args.s_arg2 + (*pr)->args.s_arg3);
	}
	else if ((*pr)->args.arg1 == DIR_CODE && (crt_p += 2))
		tmp_idx = convert_int_pos(vm, (*pr)->cur_pos, 2);
	else if ((*pr)->args.arg1 == IND_CODE && (crt_p += 2))
		tmp_idx2 = set_ind(vm, (int[]){(*pr)->cur_pos, 0});
	if ((crt_p = ldi_arg2_execut(vm, pr, (int[]){crt_p,
		tmp_idx2}, tmp_idx)) == 0)
