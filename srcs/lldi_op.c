...
/*   Updated: 2020/10/18 17:48:11 by oelbelam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			lldi_execute(t_vm *vm, t_proc **pr)
{
	int		crt_p;
	short	tmp_idx;
	int32_t	tmp_idx2;
	...
	{
		if (!check_reg(vm, i_mod((*pr)->cur_pos)))
		{
	...
	if (!(crt_p = lldi_arg2_execute(vm, pr, (int[]){crt_p, tmp_idx2}, tmp_idx)))
...
int			lldi_op(t_vm *vm, t_proc **prcs, t_proc **head, t_player **player)
{
	...
	{
		(*prcs)->cur_pos = i_mod((skip_bytes(vm->arena[(*prcs)->cur_pos], 2,
		3) + (*prcs)->cur_pos));
	}
	else
	{
