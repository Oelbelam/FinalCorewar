...
int		sti_arg3_execute(t_vm *vm, t_proc **prcs, short value, int *index)
{
	if ((*prcs)->args.arg3 == REG_CODE)
	{
		...
		+ (*prcs)->r[vm->arena[
		i_mod((*prcs)->cur_pos + 1 + index[0])] - 1]);
		index[1] = i_mod(((*prcs)->cur_pos - 2) +
		((index[1] % IDX_MOD)) + MEM_SIZE);
		index[0] += 1;
	}
	else if ((*prcs)->args.arg3 == DIR_CODE)
	{
		...
		index[1] = i_mod(((*prcs)->cur_pos - 2) + ((index[1] % IDX_MOD)) +
		MEM_SIZE);
		index[0] += 2;
	}
	if (check_reg(vm, i_mod((*prcs)->cur_pos)))
		wri_arena(vm, *prcs, index[1], (*prcs)->cur_pos);
	return (index[0]);
}
...
int		sti_execute(t_vm *vm, t_proc **prcs)
{
	...
	if ((*prcs)->args.arg2 == REG_CODE && (crt_p += 1))
	{
		...
		tmp_idx = vm->arena[((*prcs)->cur_pos + 1) % MEM_SIZE] << 8 |
		vm->arena[((*prcs)->cur_pos + 2) % MEM_SIZE] % MEM_SIZE;
	else if ((*prcs)->args.arg2 == IND_CODE && (crt_p += 2))
		tmp_idx2 = set_ind(vm, (int[]){i_mod((*prcs)->cur_pos), 1});
	if (!(crt_p = sti_arg3_execute(vm, prcs, tmp_idx,
	(int[]){crt_p, tmp_idx2})))
		return ((*prcs)->args.s_arg1 + (*prcs)->args.s_arg2 +
		(*prcs)->args.s_arg3);
		return (crt_p + 1);
